...
const loginFunc = async (req, res) => {
    ...
    try {
        const { rows } = await database.query("SELECT a.user_id, a.username, a.is_seller, a.password_hash, a.is_2fa, c.code FROM accounts AS a LEFT JOIN coupons AS c ON a.user_id = c.userid WHERE (c.active = TRUE OR c.active IS NULL) AND LOWER(a.email) = $1", [email]);
